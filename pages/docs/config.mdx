...
Use the <InlineCode>/config logging-channel</InlineCode> commands to manage the logging channel.

<Hint type="warn">
  If the webhook used for logging is deleted, and the bot does not have the
  <InlineCode>MANAGE_WEBHOOKS</InlineCode> discord permission on that channel, logs
  will fail **silently**.
</Hint>
