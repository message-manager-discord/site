...
// TODO: Tidy up and make more simple
// Also accept a list of file paths - decide if it's better to accept in props or do itself - maybe move components to routes/docs.tsx
// Investigate making the section of index.mdx that deals with table of content dynamic
// Parse these for titles
// Investigate frontmatter for the mdx files
// Investigate how to get frontmatter at build
// Use NavLink as isActive to show place
// Ensure issue where menu gets weird with state isn't a problem
// Make code as DRY as possible - including classnames

const Menu = ({ pathname }: { pathname: string }) => {
  const shared =
    "hover:text-slate-800 dark:text-slate-300 flex items-center p-4 md:p-1.5 my-2 transition-colors dark:hover:text-white  duration-200 justify-start rounded-md";
  const onlyInactive =
    "text-slate-500 font-thin hover:bg-slate-100 dark:hover:bg-slate-600";
  const onlyActive = "text-slate-600 bg-slate-100 dark:bg-slate-600";
